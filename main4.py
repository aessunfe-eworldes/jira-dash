...
# Page 2 layout with Fetch button, project selection, file picker, and graphs
...
# Callback to dynamically change button text based on project selection
@app.callback(
    Output('fetch-data-btn', 'children'),
    Input('project-selector', 'value')
)
def update_button_text(project_selection):
    if project_selection == 'local':
        return 'Load Data'
    return 'Fetch Data'
...
def update_upload_feedback(file_contents, filename):
    if file_contents is not None:
        # Show visual feedback with file name and a success icon
        return html.Div([
            html.I(className="fas fa-check-circle", style={'color': 'green', 'marginRight': '10px'}),  # Checkmark icon
            html.Span(f'Uploaded file: {filename}')  # Display file name
        ])
    return html.Div('No file uploaded yet')

    
    
# Show or hide the file picker based on the dropdown selection
@app.callback(
    Output('file-upload-container', 'style'),
    Input('project-selector', 'value')
)
def toggle_file_picker(project_selection):
    if project_selection == 'local':
...
def fetch_jira_data(n_clicks, file_contents, project_selection, file_name):
    # Trigger on button click or file upload
    if n_clicks > 0:
        df = None
        # Handle local file upload
        if project_selection == 'local' and file_contents:
            try:
                ...
                if file_name.endswith('.csv'):
                    ...
                    return dash.no_update, f"Unsupported file type: {file_name}", None, None, None, None, []

                # Debug print for loaded DataFrame
                print(f"Loaded DataFrame columns: {df.columns}")
                
            except Exception as e:
                print(f"Error processing local file: {e}")
                return dash.no_update, f"Error loading file: {e}", None, None, None, None, []

        else:
            # Handle Jira API data fetch
            try:
                print(f"Fetching data from project URL: {project_selection}")
                df = pull_from_jira_api(project_selection)  # Fetch from API
                
                print(f"Loaded DataFrame columns: {df.columns}")
            except Exception as e:
                print(f"Error fetching data: {e}")
                return dash.no_update, f"Error fetching data: {e}", None, None, None, None, []
        ...
        if df is not None:
            ...
            # Convert DataFrame to JSON format for storing in dcc.Store
            jira_data_json = df.to_json(date_format='iso', orient='split')

            return jira_data_json, 'Data fetch complete!', min_date, max_date, min_date, max_date, person_options
        else:
            return dash.no_update, 'No data to process', None, None, None, None, []
